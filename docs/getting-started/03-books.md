...
- 已完成 Core Wiki 初始化。
- 自动测试只需要 Python；可视验收需要 macOS Obsidian 与 Bases 核心插件。
...
```bash
uv run mindos books init ./my-mind-os --json
uv run mindos books init ./my-mind-os --apply --json
uv run mindos books validate ./my-mind-os --json
```
...
确认 `status` 为 `succeeded` 且 `metrics.issue_count` 为 0。随后在 Obsidian 中打开合成 vault，确认修改 `status` 能回写 frontmatter；这一步属于人工 Gate。
