...
规范 Skills 位于 `.agents/skills/`。Claude Code、Codex、Pi、Hermes、OpenClaw 与 WorkBuddy 通过各自的原生目录加载同一套 Skill。适配器只处理宿主路径和配置，不重复实现业务流程。
...
第一版只暴露 `mindos_wiki_lint`、`mindos_wiki_query`、`mindos_books_validate` 和 `mindos_wiki_init`。stdio stdout 只承载协议；工具通过子进程调用同一 CLI，写入仍需显式 `apply: true`。当前版本不提供自动 Registry、远程 transport、resources、任务状态或模型调用。
