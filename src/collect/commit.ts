...
import { assertTwitterQuality, auditTwitterDaily, auditTwitterTarget, twitterDailyEntryCount, type TwitterQualityReport } from "./audit.js";
import type { Batch, Decision, Source } from "./model.js";

export type Phase = "reserved" | "output" | "seen" | "cursor" | "applied" | "reverted"; export type Receipt = { source: Source; decision_hash: string; date: string; target: string; phase: Phase; reserved_at: number; output_hash?: string };
...
async function boundedRead(path: string, maxBytes: number): Promise<string> {
  ...
  try { return new TextDecoder("utf-8", { fatal: true }).decode(await readFile(path)); } catch { throw new MindosError("mindos.state.conflict", "collection file is not valid UTF-8"); }
}
export async function readState(root: string, name: string): Promise<JsonState> {
  const path = await resolveReadPath(root, `.mindos/collect/${name}.json`); try {
    const content = await boundedRead(path, 2 * 1024 * 1024); const value: unknown = JSON.parse(content);
...
async function writeState(root: string, name: string, state: JsonState): Promise<void> {
  state.hash = (await atomicWrite(root, `.mindos/collect/${name}.json`, `${JSON.stringify(state.value, null, 2)}\n`, { expectedHash: state.hash })).hash;
}
const inline = (value: string, limit = 32_000): string => value.slice(0, limit).replace(/[\r\n]+/gu, " ").replace(/([\\`*_[\]<>])/gu, "\\$1");
const markdownDestination = (value: string): string => `<${value.replace(/[()[\]<>\\\s]/gu, (character) => [...Buffer.from(character, "utf8")].map((byte) => `%${byte.toString(16).toUpperCase().padStart(2, "0")}`).join(""))}>`;
...
function twitterAuthor(url: string, fallback = "source"): string {
  try { const parsed = new URL(url); const handle = parsed.hostname === "x.com" ? parsed.pathname.split("/")[1] : undefined; if (handle !== undefined && /^[A-Za-z0-9_]{1,50}$/u.test(handle)) return handle; }
  catch { /* URL 已在 Provider 规范化时校验；这里只使用安全回退。 */ }
  const author = fallback.trim().replace(/^@/u, ""); return /^[A-Za-z0-9_]{1,50}$/u.test(author) ? author : "source";
}
...
function normalizeTwitterSection(section: string): string {
  ...
  return section.replace(detailed, (_match, id: string, title: string, summary: string, url: string) => `${marker("twitter", id)}\n${number += 1}. **${title}**：${summary}\n   — [@${twitterAuthor(url)}](${markdownDestination(url)})`);
}
...
function normalizeRssSection(section: string, sources: Map<string, string>): string {
  ...
  return section.replace(detailed, (_match, id: string, title: string, summary: string, url: string) => `${marker("rss", id)}\n${number += 1}. **${title}**：${summary}\n   — [${inline(rssSource(url, sources.get(id)), 200)}](${markdownDestination(url)}) · Folo entry \`${id}\``);
}
...
function initialDaily(source: Source, date: string, now: number): string {
  const count = source === "twitter" ? "tweet_count" : "entry_count"; const title = source === "twitter" ? `X/Twitter 每日信息简报 — ${date}` : `${date} - Folo精选信息简报`; const origin = source === "twitter" ? "x.com/home" : "folo";
...
function updateDailyMetadata(source: Source, content: string, now: number): string {
  ...
  let size: number; if (source === "twitter") size = twitterDailyEntryCount(content); else { const ids = new Set<string>(); for (const match of content.matchAll(/mindos:collect:rss:([\w.:-]+)/gu)) ids.add(match[1] ?? ""); for (const match of content.matchAll(/Folo entry `([\w.:-]+)`/gu)) ids.add(match[1] ?? ""); ids.delete(""); size = ids.size; }
  const count = source === "twitter" ? "tweet_count" : "entry_count";
  const time = new Date(now).toLocaleTimeString("en-GB", { hour: "2-digit", minute: "2-digit", hour12: false });
  return content.replace(new RegExp(`^${count}: \\d+$`, "mu"), `${count}: ${size}`)
    .replace(/^last_updated: .*$/mu, `last_updated: "${time}"`);
}

function validateDecisions(batch: Batch, input: DecisionInput): Map<string, Decision> {
  if (input.baseline_hash !== batch.baseline_hash || input.decisions.length !== batch.signals.length) throw new MindosError("mindos.input.invalid", "decisions do not match the collection batch");
  const signals = new Map(batch.signals.map((signal) => [signal.id, signal])); const expected = new Set(signals.keys()); const decisions = new Map<string, Decision>();
  for (const decision of input.decisions) {
    const keep = decision.decision === "keep";
    if (!expected.has(decision.id) || decisions.has(decision.id) || decision.reason.trim().length === 0 || decision.reason.length > 2_000) throw new MindosError("mindos.input.invalid", "collection decisions are invalid");
    if (keep && (decision.display_title?.trim() === "" || (decision.display_title?.length ?? 0) > 500 || decision.display_summary?.trim() === "" || (decision.display_summary?.length ?? 0) > 32_000 || decision.category === undefined || !(decision.category in batch.config.categories) || decision.tags?.some((tag) => tag.trim() === "" || tag.length > 80))) throw new MindosError("mindos.input.invalid", "collection decision display fields are invalid");
    decisions.set(decision.id, decision);
  }
  const kept = [...decisions.values()].filter((decision) => decision.decision === "keep"); const bareShortlink = /^(?:[\s\p{Extended_Pictographic}\uFE0F]*)https:\/\/t\.co\/[A-Za-z0-9]+$/u;
  for (const decision of kept) {
    const title = decision.display_title?.trim() ?? ""; const summary = decision.display_summary?.trim() ?? ""; const signal = signals.get(decision.id); const sourceHasHan = /\p{Script=Han}/u.test(`${signal?.title ?? ""}\n${signal?.content ?? ""}`);
    if (bareShortlink.test(title) || bareShortlink.test(summary) || (title === summary && title.length <= 40) || (batch.source === "twitter" && ((!/\p{Script=Han}/u.test(title) || !/\p{Script=Han}/u.test(summary)) || (!sourceHasHan && decision.translated !== true) || /^(?:关于.{1,400}的(?:探讨|讨论)[：:]?|(?:探讨|讨论|分享|解读)[：:])/u.test(title) || /^(?:(?:作者)?(?:分享|介绍)了?如下内容[：:]|(?:核心)?要点(?:如下)?[：:])/u.test(summary)))) throw new MindosError("mindos.input.invalid", "collection decisions failed semantic quality checks");
  }
...
async function readDaily(root: string, relative: string): Promise<{ content: string; hash: string | null }> {
  const path = await resolveWritePath(root, relative); try { const content = await boundedRead(path, 8 * 1024 * 1024); return { content, hash: contentHash(Buffer.from(content, "utf8")) }; }
  catch (error: unknown) {
...
function renderDaily(batch: Batch, decisions: Map<string, Decision>, date: string, existing: string, now: number): string {
  let output = existing || initialDaily(batch.source, date, now); output = batch.source === "twitter" ? normalizeTwitterManaged(output) : normalizeRssManaged(output, batch);
  const additions = batch.signals.filter((signal) => decisions.get(signal.id)?.decision === "keep" && !output.includes(marker(batch.source, signal.id)) && !output.includes(signal.url));
  ...
  for (const [categoryIndex, [category, label]] of categories.entries()) {
    const grouped = additions.filter((signal) => decisions.get(signal.id)?.category === category); if (grouped.length === 0) continue;
    ...
    if (headingIndex < 0) {
      ...
      output = laterHeading === undefined ? `${output.trimEnd()}\n\n${heading}\n` : `${output.slice(0, laterHeading).trimEnd()}\n\n${heading}\n\n${output.slice(laterHeading)}`;
...
export function parseCollectionDecisions(value: unknown): DecisionInput { return parseContract("collectionDecisions", value, "collection decisions are invalid"); }
...
export async function commitCollection(root: string, source: Source, input: DecisionInput, options: CommitOptions): Promise<CommitOutcome> {
  ...
  try {
    ...
    if (receipt?.phase === "applied") {
      const quality = source === "twitter" ? await auditTwitterTarget(root, receipt.target) : undefined;
      if (quality !== undefined) assertTwitterQuality(quality);
      return { changed: false, target: receipt.target, data: { batch_id: input.batch_id, replay: true, ...(quality === undefined ? {} : { quality }) }, artifacts: [] };
    }
    const batch = await loadBatch(root, input.batch_id, source, receipt !== undefined || options.apply, now);
    if (receipt === undefined && options.apply && now - batch.created_at > 30 * 86_400_000) {
      throw new MindosError("mindos.state.batch_expired", "collection recovery batch has expired");
    }
    const decisions = validateDecisions(batch, input);
    ...
    let quality: TwitterQualityReport | undefined;
    if (source === "twitter") {
      const keptIds = new Map([...decisions.values()].filter((item) => item.decision === "keep").map((item) => {
        const signal = batch.signals.find((candidate) => candidate.id === item.id);
        const legacyId = signal?.url.match(/^https:\/\/x\.com\/[^/]+\/status\/(\d+)(?:$|[?#])/u)?.[1];
        return [item.id, legacyId] as const;
      }));
      const discardedIds = new Set([...decisions.values()].filter((item) => item.decision === "discard").map((item) => item.id));
      quality = auditTwitterDaily(rendered, { kept: keptIds, discarded: discardedIds }); assertTwitterQuality(quality);
    }
