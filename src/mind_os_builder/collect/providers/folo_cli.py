...
class FoloCliProvider:
    name = "folo-cli"

    def __init__(
        self,
        *,
        command: tuple[str, ...] = ("folocli", "entries", "--json"),
        timeout: float = 30.0,
    ...
    def capability(self) -> ProviderCapability:
        return ProviderCapability(source="rss", network=True, experimental=True)
    ...
    def fetch(self, cursor: str | None = None) -> ProviderBatch:
        ...
        return run_json_command(
            ...
            runner=self._runner,
            record_keys=("entries", "records", "items"),
        )
