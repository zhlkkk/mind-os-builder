...
1. 检查 `mindos doctor --json` 和 `<vault-root>/.mindos/config.yaml` 中的 `collect.rss` 配置；完成条件：Folo CLI 可执行，`output_directory`、`daily_filename`、过滤规则和分类表已确认，凭证没有进入仓库或命令参数。
...
7. 仅在用户确认或已授权的自动任务中追加 `--apply`；完成条件：每日简报、seen 和提交回执一起收敛，失败时可用同一批次重试。
8. 重放同一决策文件并重新 prepare；完成条件：重放为 `state: noop`，仍在最新窗口中的已处理条目不再进入下一批候选。
