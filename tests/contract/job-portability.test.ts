...
import { loadJobs } from "../../src/jobs/catalog.js";
import { exportJob } from "../../src/jobs/export.js";
...
test("全部内置 Job 可导出为符合契约的 Agent 任务", async () => {
  const schema = JSON.parse(await readFile(join(root, "contracts/agent-job.schema.json"), "utf8")) as object;
  const validate = new Ajv({ allErrors: true }).compile(schema);
  for (const job of await loadJobs()) {
    const inputs = Object.keys(job.inputs).map((key) => `${key}=synthetic-${key}`);
    const exported = exportJob(job, { adapter: "agent", inputs });
    assert.equal(exported.media_type, "application/json");
    assert.equal(validate(JSON.parse(exported.content)), true, `${job.id}: ${JSON.stringify(validate.errors)}`);
  }
});
